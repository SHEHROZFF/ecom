...
// 1) Import Redux pieces
import { useDispatch } from 'react-redux';
import { fetchCoursesThunk, searchCoursesThunk } from '../store/slices/courseSlice';

// We keep the pagination limit and header height the same
const PAGE_LIMIT = 10;
const HEADER_HEIGHT = 220;

/* ---------------------------------------------------------------------------
   1) AICoursesHeader - local search logic updated to use `searchCoursesThunk`
   --------------------------------------------------------------------------- */
const AICoursesHeader = memo(function AICoursesHeader({
  ...
  const dispatch = useDispatch(); // For searching via Redux
...
const AICoursesScreen = () => {
  ...
  // 6) Replacing the old fetchCourses() call with a Redux thunk
