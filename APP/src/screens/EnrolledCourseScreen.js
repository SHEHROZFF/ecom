...
import {
  fetchCourseById,
  getMyEnrollmentsAPI,
  updateLessonProgressAPI, // NEW API call for lesson progress
} from '../services/api';
...
const EnrolledCourseScreen = () => {
  ...
  // Open lesson modal; now we use the video’s _id as identifier
  const handleLessonPress = (lesson) => {
    setSelectedLesson(lesson); // lesson now includes _id, title, url, etc.
  ...
  // Update the enrollment record with lesson progress using the new video _id
  const markLessonCompleted = async (lesson) => {
    ...
    // Call new API endpoint for updating lesson progress
