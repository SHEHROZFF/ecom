...
const enrollInCourse = asyncHandler(async (req, res) => {
  ...
  // });
  // Fetch all video lessons for tracking progress
  const lessons = await VideoLesson.find({ course: courseId });

  // 3. Create new enrollment with lesson tracking
  const enrollment = await Enrollment.create({
    user: userId,
    course: courseId,
...
const updateLessonProgress = asyncHandler(async (req, res) => {
  ...
  enrollment.lessonsProgress = enrollment.lessonsProgress.map(lp => {
    ...
    if (lp.lessonId.toString() === lessonId) {
  ...
  const course = await Course.findById(courseId);
  const totalLessons = await VideoLesson.countDocuments({ course: courseId });
  const numCompleted = enrollment.lessonsProgress.filter(lp => lp.completed).length;
  enrollment.progress = Math.min((numCompleted / totalLessons) * 100, 100); // Ensure 0-100%
